...
#             print(labels_arr, len(labels_arr))
    ...
    multi_hot_labels_df = pd.DataFrame(
        dict_list, columns=(['ImageID'] + unique_labels_list))
...
def img_to_h5(
    ...
    with h5py.File(out_filepath, 'w') as h5f:
        img_dset = h5f.create_dataset(
            'cxr', shape=(dset_size, resolution, resolution))
...
def write_h5(cxr_paths, resolution: int = 320):
    ...
    with h5py.File(out_filepath, 'w') as h5f:
        ...
        img_dset = h5f.create_dataset(
            'cxr', shape=(2978, resolution, resolution))
